...
func (mc *MediaConvert) Transcode(ctx context.Context, args TranscodeJobArgs) (outs []video.OutputVideo, err error) {
	...
	var (
		...
		thumbsTarget = args.ThumbsOutputLocation
	)
	...
	var mcThumbsOutputRelPath string
	if thumbsTarget != nil {
		mcThumbsOutputRelPath = path.Join("output", getTargetDir(thumbsTarget, args.RequestID, "thumbnails"))
		mcArgs.ThumbsOutputLocation = mc.s3TransferBucket.JoinPath(mcThumbsOutputRelPath, "keyframes")
	}
	...
	if thumbsTarget != nil {
		// TODO rename thumbs generated by mediaconvert. they are created with names like keyframes.0000001.jpg whereas we want keyframes_1.jpg

		mcThumbsOutputBaseDir := mc.osTransferBucketURL.JoinPath(mcThumbsOutputRelPath, "..")
		log.Log(args.RequestID, "Copying thumbs output files from S3", "source", mcThumbsOutputBaseDir, "dest", thumbsTarget)
		if err := copyDir(mcThumbsOutputBaseDir, thumbsTarget, args); err != nil {
			return nil, fmt.Errorf("error copying output files: %w", err)
		}
	}
...
func (mc *MediaConvert) coreAwsTranscode(ctx context.Context, args TranscodeJobArgs, accelerated bool) (err error) {
	...
	payload := createJobPayload(args.InputFile.String(), toStr(args.HLSOutputLocation), mp4OutputLocation, args.ThumbsOutputLocation.String(), mc.role, accelerated, args.Profiles, args.SegmentSizeSecs)
...
func createJobPayload(inputFile, hlsOutputFile, mp4OutputFile, thumbsOutput, role string, accelerated bool, profiles []video.EncodedProfile, segmentSizeSecs int64) *mediaconvert.CreateJobInput {
	...
	return &mediaconvert.CreateJobInput{
		Settings: &mediaconvert.JobSettings{
			...
			OutputGroups: outputGroups(hlsOutputFile, mp4OutputFile, thumbsOutput, profiles, segmentSizeSecs),
...
func outputGroups(hlsOutputFile, mp4OutputFile, thumbsOutput string, profiles []video.EncodedProfile, segmentSizeSecs int64) []*mediaconvert.OutputGroup {
	groups := []*mediaconvert.OutputGroup{
		{
			CustomName: aws.String("thumbs"),
			...
			OutputGroupSettings: &mediaconvert.OutputGroupSettings{
				FileGroupSettings: &mediaconvert.FileGroupSettings{
					Destination: aws.String(thumbsOutput),
				},
			...
			Outputs: []*mediaconvert.Output{
				{
					ContainerSettings: &mediaconvert.ContainerSettings{
						Container: aws.String(mediaconvert.ContainerTypeRaw),
					},
					VideoDescription: &mediaconvert.VideoDescription{
						...
						CodecSettings: &mediaconvert.VideoCodecSettings{
							Codec: aws.String(mediaconvert.VideoCodecFrameCapture),
							FrameCaptureSettings: &mediaconvert.FrameCaptureSettings{
								FramerateNumerator:   aws.Int64(1),
								FramerateDenominator: aws.Int64(10),
							},
						},
					},
				},
