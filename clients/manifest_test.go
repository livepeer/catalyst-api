...
func TestItCanGenerateAndWriteFmp4Manifests(t *testing.T) {
	// Set up the parameters we pass in
	outputDir, err := os.MkdirTemp(os.TempDir(), "TestItCanGenerateAndWriteManifests-*")
	require.NoError(t, err)

	fmp4Manifests := map[string]string{
		"super-high-def":          "super-high-def.m3u8",
		"bit-more-super-high-def": "bit-more-super-high-def.m3u8",
	}
	...
	// Confirm we wrote out the master manifest that we expected
	fmp4MasterManifest := filepath.Join(outputDir, "/"+FMP4_POSTFIX_DIR, "index.m3u8")
	require.FileExists(t, fmp4MasterManifest)
	fmp4MasterManifestContents, err := os.ReadFile(fmp4MasterManifest)
	require.NoError(t, err)

	const expectedFmp4MasterManifest = `#EXTM3U
#EXT-X-VERSION:3
#EXT-X-STREAM-INF:PROGRAM-ID=0,BANDWIDTH=1,RESOLUTION=2560x1440,NAME="1-bit-more-super-high-def",FRAME-RATE=30.000
bit-more-super-high-def/bit-more-super-high-def.m3u8
#EXT-X-STREAM-INF:PROGRAM-ID=0,BANDWIDTH=1,RESOLUTION=1080x720,NAME="2-super-high-def",FRAME-RATE=30.000
super-high-def/super-high-def.m3u8
`
	require.Equal(t, expectedFmp4MasterManifest, string(fmp4MasterManifestContents))
